...
const NotificationScreen = ({ navigation }) => {
  ...
  const getUserPrompts = async () => {
    var id = await AsyncStorageLib.getItem(AS_KEYS.TOKEN);

    const { Prompts } = await getData(COLLECTIONS.PROMPTS, id);
    setListData(Prompts != undefined ? Prompts : []);
  };
  useEffect(() => {
    getPromptList();
    getUserPrompts();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.subtitleview}>
        ...
        <Text style={styles.subtitle}>Edit Prompts</Text>
      </View>
